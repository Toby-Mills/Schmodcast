...
class NowPlayingWidgetProvider : AppWidgetProvider() {
    ...
    private fun placeholderViews(context: Context): RemoteViews =
        RemoteViews(context.packageName, R.layout.widget_now_playing).apply {
            ...
            setOnClickPendingIntent(R.id.widget_play_pause, openApp)
            setOnClickPendingIntent(R.id.widget_skip_back, openApp)
            setOnClickPendingIntent(R.id.widget_skip_forward, openApp)
        }
    ...
    companion object {
        ...
        // Plain getService, not getForegroundService - these are simple control taps, and
        // PlaybackService/Media3 already promotes itself to a foreground service internally the
        // moment real playback starts, so this doesn't need to (and shouldn't) race that. Each
        // button gets its own requestCode so their PendingIntents don't collide.
