...
`EpisodeEntity.lastPositionMs` persists each episode's seek point, and `PlaybackStateStore.currentEpisodeId` persists which episode was last loaded (independent of queue order — it can be an episode picked out of turn, not just the head). `PlaybackService` writes both: `loadEpisode` sets `playbackStateStore.currentEpisodeId` whenever any episode loads (head, auto-advance, or out-of-order tap), and `lastPositionMs` is written via a `Player.Listener` — a 5s ticker while playing, immediately on pause (`onIsPlayingChanged`) and on any seek (`onPositionDiscontinuity`, which fires regardless of whether the seek came from the skip buttons, the progress slider, or a fresh `loadEpisode`'s own resume-seek), plus a final synchronous save in `onDestroy` (using `runBlocking`, since `serviceScope.cancel()` would otherwise cancel a just-launched save coroutine before it ran). On the first queue emission after process start, the service looks up `playbackStateStore.currentEpisodeId` in the freshly-collected queue and loads that episode (seeking to its `lastPositionMs`) instead of unconditionally loading the queue head; if that episode is no longer in the queue (played/pruned), it falls back to the head. `EpisodeRepository.refreshFeed` preserves both `localFilePath` and `lastPositionMs` across a feed refresh (by copying them from the pre-existing row before `insertAll`'s REPLACE), since `QueueViewModel.init` triggers a refresh on every app launch — without that preservation, resuming a position would get silently reset to 0 by the very refresh that runs before playback restores.
